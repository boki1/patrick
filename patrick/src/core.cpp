    ...
    : m_generator{ generator_matrix }
{
  if (m_generator.isZero())
      throw linearcode_exception("Cannot instantiate a linearcode from the empty matrix.");
...
[[nodiscard]] syndrome linearcode::syndrome_of(const codeword &cword) const
{
  if (cword.vec.cols() != m_generator.cols())
      throw linearcode_exception{fmt::format("Codeword '{}' has incompatible dimensions to be part "
                                             "of a code, whose generator matrix has {} columns.",
                                             cword,
                                             m_generator.cols())};
  const auto &H = parity_matrix();
  const auto product = cword.vec.operator*(H.transpose()).unaryExpr([&](const int x) {
      return x % 2;
  });
  return syndrome{std::move(product)};
}

[[nodiscard]] bool linearcode::contains(const codeword &cword) const
{
  try {
...
void linearcode::evaluate_properties_of()
{
  prepare_codewords();
...
void linearcode::prepare_codewords() const
{
  ...
  for (std::size_t iword_as_num = 0; iword_as_num < total_codeword_count; ++iword_as_num)
...
void linearcode::prepare_parity_matrix() const
{
...
linearcode::encode (const infoword &iword) const
{
  ...
  if (iword.vec.cols() != m_generator.rows())
    throw linearcode_exception{
        fmt::format("Trying to encode infoword '{}' which has size n={}, whereas the code "
                    "expects n={}.",
                    iword,
                    iword.vec.cols(),
                    m_generator.rows())};
  const auto product = iword.vec.operator*(m_generator).unaryExpr([&](const int x) {
      return x % 2;
  });
...
/// 5. Else, go to 2.
///
void linearcode::prepare_slepian_table() const
{
  ...
  // The first row (coset) of the Slepian table contains the codewords themselves.
  slepian_table.emplace_back ([&] () {
    std::vector<codeword> non_null_codewords;
    non_null_codewords.resize (m_lazy_codewords->size () - 1);
	std::copy(m_lazy_codewords->cbegin() + 1, m_lazy_codewords->cend(), non_null_codewords.begin());
  ...
  for (std::size_t i = 1; i < num_rows; ++i) {
    ...
    std::transform(table_header_words.cbegin(),
                   table_header_words.cend(),
    ...
    slepian_table.emplace_back(coset{.leader = leader, .columns = std::move(words)});
  }

  m_lazy_slepian_table.emplace(std::move(slepian_table));
}

[[nodiscard]] linearcode::decoding_result linearcode::decode_with_slepian(const codeword &cword)
{
  if (!m_lazy_slepian_table)
    prepare_slepian_table();

  /// Safety: The only way for the table to have no rows could be only due to the generator being empty.
  /// The generator is non-zero as asserted in the ctor.
  assert(m_lazy_slepian_table->size() > 0);

  const codeword &topleft = m_lazy_slepian_table->front().leader;
  const std::vector<codeword> &codewords = m_lazy_slepian_table->front().columns;
  ...
  for (const coset &row : *m_lazy_slepian_table) {
    ...
    if (it != std::cend(row.columns)) {
        const std::size_t column_idx = std::distance(std::begin(row.columns), it);
  ...
  /// Safety: cword is of size that may be part of the linear code subspace. The
  /// Slepian table contains all words that are part of \f$F_{2}^{n}\f$, so at
  /// some point the `find()` inside the loop body will reach it.
  assert(corrected_cword.vec.size() > 0 && correction.vec.size() > 0);

  const std::size_t k = m_properties.basis_size;
  const std::size_t t = m_properties.max_errors_detect;

  if (const auto num_errors_found = correction.weight(); num_errors_found > t)
    throw linearcode_exception{
        fmt::format("Cannot decode codeword '{}' because more errors were "
                    "found then the maximum that could be recovered - {} > {}.",
                    cword,
                    num_errors_found,
                    t)};

  /// The conversion from a codeword of size N to its infoword of size K is based on the fact
  /// that the encoding is systematic, meaning that the positions of the codeword which contain
  /// information are the same as kept the same as in the infoword. Therefore, we have to _cut_
